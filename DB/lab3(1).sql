...
INSERT INTO [Рейсы] ([Номер рейса],[ID маршрута]) VALUES 
('SU 1234', 
(SELECT [ID маршрута]
FROM [Маршрут]
WHERE [Пункт вылета (код терминала)]=
										(SELECT [ID терминала]
										FROM [Терминалы]
										WHERE [ID аэропорта]= (
																SELECT [ID аэропорта]
										...
										AND [Название терминала]='F')
AND [Пункт назначения (код терминала)] =
										(SELECT [ID терминала]
										FROM [Терминалы]
										WHERE [ID аэропорта]= (
																SELECT [ID аэропорта]
